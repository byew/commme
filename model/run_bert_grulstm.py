...
import json
import sys
sys.path.append("/home/baiyang/smm5")
from pytorch_transformers.modeling_bert import BertForSequenceClassification_grulstm, BertConfig
from pytorch_transformers import AdamW, WarmupLinearSchedule, Lamb, RAdam
...
logging.basicConfig(format = '%(asctime)s - %(levelname)s - %(name)s -   %(message)s',
                    ...
                    level = logging.INFO)
MODEL_CLASSES = {
    'bert': (BertConfig, BertForSequenceClassification_grulstm, BertTokenizer),
}
opp = {
    'AdamW':(AdamW),
    'RAdam': (RAdam),
...
def main():
    ...
    parser.add_argument("--op", default=None, type=str, required=True,
                        help="Model type selected in the list: " + ", ".join(opp.keys()))
    ...
    # Prepare model
    model = BertForSequenceClassification_grulstm.from_pretrained(args.model_name_or_path,args,config=config)
    ...
    if args.do_train:
        ...
        youhuaqi = opp[args.op]
        # optimizer = AdamW(optimizer_grouped_parameters, lr=args.learning_rate, eps=args.adam_epsilon)
        # optimizer = Lamb(optimizer_grouped_parameters, lr=args.learning_rate, eps=args.adam_epsilon)
        optimizer = youhuaqi(optimizer_grouped_parameters, lr=args.learning_rate, eps=args.adam_epsilon)
    ...
    if args.do_test:
        ...
        gc.collect()
        args.do_train=False
        model = BertForSequenceClassification_grulstm.from_pretrained(os.path.join(args.output_dir, "pytorch_model.bin"),args,config=config)
        if args.fp16:
            model.half()
        model.to(device)
        if args.local_rank != -1:
        ...
        for file,flag in [('dev.csv','dev'),('test.csv','test')]:
            ...
            if flag=='test':
                ...
                df[['id', 'label_0', 'label_1']].to_csv(os.path.join(args.output_dir, "sub.csv"),
                                                                   index=False)
            if flag == 'dev':
                df = pd.read_csv(os.path.join(args.data_dir, file))
                df['label_0']=logits[:,0]
                ...
                df[['id', 'label_0', 'label_1']].to_csv(os.path.join(args.output_dir, "sub_dev.csv"),
                                                                   index=False)
    ...
    if args.predict_eval:
        ...
        gc.collect()
        args.do_train = False
        model = BertForSequenceClassification_grulstm.from_pretrained(os.path.join(args.output_dir, "pytorch_model.bin"), args,
                                                              config=config)
        if args.fp16:
            model.half()
        model.to(device)
