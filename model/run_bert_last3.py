...
from pytorch_transformers.modeling_bert import BertForSequenceClassification_last3, BertConfig
...
logging.basicConfig(format = '%(asctime)s - %(levelname)s - %(name)s -   %(message)s',
                    ...
                    level = logging.INFO)
MODEL_CLASSES = {
    'bert': (BertConfig, BertForSequenceClassification_last3, BertTokenizer),
}
...
def main():
    ...
    # Prepare model
    model = BertForSequenceClassification_last3.from_pretrained(args.model_name_or_path,args,config=config)
    ...
    if args.do_test:
        ...
        gc.collect()
        args.do_train=False
        model = BertForSequenceClassification_last3.from_pretrained(os.path.join(args.output_dir, "pytorch_model.bin"),args,config=config)
        if args.fp16:
            model.half()
        model.to(device)
        if args.local_rank != -1:
    ...
    if args.predict_eval:
        ...
        gc.collect()
        args.do_train = False
        model = BertForSequenceClassification_last3.from_pretrained(os.path.join(args.output_dir, "pytorch_model.bin"), args,
                                                              config=config)
        if args.fp16:
            model.half()
        model.to(device)
